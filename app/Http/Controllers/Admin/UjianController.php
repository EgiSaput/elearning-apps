...
class UjianController extends Controller
{
  ...
  public function ambilUjian($id_ujian)
  {
    $ujian = Ujian::find($id_ujian);
    $siswa = Siswa::where('id_user', Auth::user()->id_user)->first();
    if (!$ujian || !$siswa) {
      return Redirect::back()->with('flash_message', 'Data ujian atau siswa tidak ditemukan.');
    }
    
    // Cek waktu ujian
    $now = now();
    $jamMulai = $ujian->jam_mulai ?? '00:00:00';
    $jamSelesai = $ujian->jam_selesai ?? '23:59:59';
    $waktuMulai = \Carbon\Carbon::parse($ujian->tgl_ujian . ' ' . $jamMulai);
    $waktuSelesai = \Carbon\Carbon::parse($ujian->tgl_ujian . ' ' . $jamSelesai);
    
    // Jika jam_selesai <= jam_mulai, berarti jam selesai di hari berikutnya
    if ($waktuSelesai <= $waktuMulai) {
      $waktuSelesai->addDay();
    }
    
    if ($now < $waktuMulai) {
      return Redirect::back()->with('flash_message', 'Ujian belum dimulai. Waktu ujian: ' . $waktuMulai->format('d F Y H:i'));
    }
    if ($now > $waktuSelesai) {
      return Redirect::back()->with('flash_message', 'Waktu ujian sudah berakhir.');
    }
    
    $mapel = MataPelajaran::find($ujian->id_mapel);
    $nama_mapel = $mapel ? $mapel->nama_mapel : '-';
    return view('admin.dashboard.ujian.konfirmasi_ambil_ujian', compact('ujian', 'siswa', 'nama_mapel'));
  }

  // Simpan pengambilan ujian oleh siswa
  public function simpanAmbilUjian(Request $request, $id_ujian)
  {
    $ujian = Ujian::find($id_ujian);
    $siswa = Siswa::where('id_user', Auth::user()->id_user)->first();
    if (!$ujian || !$siswa) {
    ...
    // Simpan pengambilan jika belum pernah ambil
    $sudahAmbil = DB::table('pengambilan_ujians')
      ->where('id_ujian', $id_ujian)
      ->where('nisn_siswa', $siswa->nisn_siswa)
      ->first();
    if (!$sudahAmbil) {
      DB::table('pengambilan_ujians')->insert([
        'id_ujian' => $id_ujian,
        'nisn_siswa' => $siswa->nisn_siswa,
        'diambil_pada' => now(),
      ]);
    }
    // Buat record nilai_ujian_pilgan_siswas jika belum ada

    $nilai = \App\NilaiUjianPilihanGandaSiswa::where('id_ujian', $id_ujian)
      ->where('nisn_siswa', $siswa->nisn_siswa)
      ->first();
    if (!$nilai) {
      $nilai = new \App\NilaiUjianPilihanGandaSiswa();
      $nilai->id_ujian = $id_ujian;
      $nilai->nisn_siswa = $siswa->nisn_siswa;
      $nilai->nilai = 0;
      $nilai->wkt_selesai = null;
      $nilai->save();
    } else {
      // Reset wkt_selesai jika record sudah ada
      $nilai->wkt_selesai = null;
      $nilai->save();
    }
    $id_nilai_ujian_pilgan = $nilai->id_nilai_ujian_pilgan;

    // Selalu hapus dan insert ulang data siswa_jawab_ujian_pilgans agar konsisten
    \App\SiswaJawabUjianPilihanGanda::where('id_nilai_ujian_pilgan', $id_nilai_ujian_pilgan)->delete();
    $soalList = \App\Soal::where('id_ujian', $id_ujian)->orderBy('id_soal')->get();
    foreach ($soalList as $soal) {
      \App\SiswaJawabUjianPilihanGanda::create([
        'id_nilai_ujian_pilgan' => $id_nilai_ujian_pilgan,
        'id_soal' => $soal->id_soal,
      ]);
    }

    return Redirect::route('siswa.ujian.index')->with('flash_message', 'Ujian berhasil diambil. Silakan mulai mengerjakan.');
  }
  ...
  public function mulaiUjian($id_ujian)
  {
    ...
    if ($now < $waktuMulai) {
      return Redirect::route('siswa.ujian.index')->with('flash_message', 'Ujian belum dimulai. Waktu ujian: ' . $waktuMulai->format('d F Y H:i'));
    }
    if ($now > $waktuSelesai) {
      return Redirect::route('siswa.ujian.index')->with('flash_message', 'Waktu ujian sudah berakhir.');
    }
    ...
    $nilai = \App\NilaiUjianPilihanGandaSiswa::where('id_ujian', $id_ujian)
      ->where('nisn_siswa', $siswa->nisn_siswa)
      ->first();
    if ($nilai && $nilai->wkt_selesai !== null) {
      return Redirect::route('siswa.ujian.index')->with('flash_message', 'Anda sudah menyelesaikan ujian ini. Tidak dapat mengerjakan ulang.');
    }

    $mapel = MataPelajaran::find($ujian->id_mapel);
    $nama_mapel = $mapel ? $mapel->nama_mapel : '-';
    return view('admin.dashboard.ujian.konfirmasi_mulai_ujian', compact('ujian', 'siswa', 'nama_mapel'));
  }

  // Simpan mulai ujian
  public function simpanMulaiUjian(Request $request, $id_ujian)
  {
    $ujian = Ujian::find($id_ujian);
    $siswa = Siswa::where('id_user', Auth::user()->id_user)->first();
    if (!$ujian || !$siswa) {
    ...
    $nilai = \App\NilaiUjianPilihanGandaSiswa::where('id_ujian', $id_ujian)
      ...
      ->first();
    if (!$nilai) {
      return Redirect::back()->with('flash_message', 'Anda belum mengambil ujian ini.');
    }

    // Cek apakah ujian sudah diselesaikan
    if ($nilai->wkt_selesai !== null) {
      return Redirect::route('siswa.ujian.index')->with('flash_message', 'Anda sudah menyelesaikan ujian ini. Tidak dapat mengerjakan ulang.');
    }

    // Cari soal pertama yang belum dijawab
    $soalBelum = \App\SiswaJawabUjianPilihanGanda::where('id_nilai_ujian_pilgan', $nilai->id_nilai_ujian_pilgan)
      ->whereNull('id_jawaban_soal_ujian')
      ->orderBy('id_soal')
      ->first();

    if ($soalBelum) {
      return Redirect::route('siswa.soal_ujian.show', [$nilai->id_nilai_ujian_pilgan, $soalBelum->id_soal]);
    } else {
      // Jika semua soal sudah dijawab, cari soal pertama
      $firstSoal = \App\Soal::where('id_ujian', $id_ujian)->orderBy('id_soal')->first();
      if ($firstSoal) {
        return Redirect::route('siswa.soal_ujian.show', [$nilai->id_nilai_ujian_pilgan, $firstSoal->id_soal]);
      }
    }

    return Redirect::route('siswa.ujian.index')->with('flash_message', 'Tidak ada soal untuk ujian ini.');
  }
...
public function showTambahUjian()
  {
    ...
    return view('admin.dashboard.ujian.tambah_ujian')
      ...
      $mapels = $guru ? $guru->mataPelajaran()->orderBy('nama_mapel')->get() : collect();
      $mapelIds = $mapels->pluck('id_mapel')->toArray();
      $dataKelas = [];
      if (count($mapelIds)) {
...
public function detail($id_ujian)
  {
    ...
    // dd($dataUjian);
    // revisi check waktu ujian - hanya untuk siswa (level 13)
    $wkt_ujian  = $dataUjian->tgl_ujian;
    $wkt_sekarang = Date('Y-m-d');
    if ($wkt_sekarang > $wkt_ujian && Auth::user()->level == 13) {
          Session::flash('flash_message', 'Batas akhir ujian sudah berakhir. Silakan hubungi guru yang bersangkutan.');
    ...
    $dataSoalUjianDetail = DB::table('soals')
         ...
         ->leftjoin('jawaban_soal_ujians', 'soals.id_soal', '=', 'jawaban_soal_ujians.id_soal')
         ->select('soals.*', 'ujians.*', 'mata_pelajarans.nama_mapel', 'gurus.nama_guru', DB::raw('MAX(jawaban_soal_ujians.poin) as max_poin'))
         ->where('soals.id_ujian', $id_ujian)
         ->groupBy('soals.id_soal')
         ->get();
    ...
    $dataJawabanSoal = DB::table('jawaban_soal_ujians')
         ...
         ->select('jawaban_soal_ujians.poin', 'jawaban_soal_ujians.id_soal', 'soals.poin as soal_poin', 'soals.jenis_soal')
         ->get();

    $maxPoint = 0;
    foreach ($dataJawabanSoal as $jawaban){
        $poin = $jawaban->jenis_soal == 'Essay' ? $jawaban->soal_poin : $jawaban->poin;
        if ($poin > $maxPoint)
            $maxPoint = (int)$poin;
        }
...
public function index()
  {
    if (Auth::user()->level == 11) {
      ...
      $guru = Guru::where('id_user', Auth::user()->id_user)->first();
      $mapels = $guru ? $guru->mataPelajaran()->get()->pluck('id_mapel')->toArray() : [];
      $dataUjian = [];
      if (count($mapels)) {
        $dataUjian = DB::table('ujians')
                   ->join('mata_pelajarans', 'ujians.id_mapel', '=', 'mata_pelajarans.id_mapel')
                   ->select('ujians.*', 'mata_pelajarans.nama_mapel')
                   ->whereIn('ujians.id_mapel', $mapels)
                   ->get();
      }
    }
...
public function index_siswa()
  {
    ...
    // Ambil daftar pengambilan ujian dari tabel pengambilan_ujians
    $userAmbilUjians = DB::table('pengambilan_ujians')
      ->join('ujians', 'pengambilan_ujians.id_ujian', '=', 'ujians.id_ujian')
      ->join('mata_pelajarans', 'ujians.id_mapel', '=', 'mata_pelajarans.id_mapel')
      ->leftJoin('nilai_ujian_pilgan_siswas', function($join) {
          $join->on('pengambilan_ujians.id_ujian', '=', 'nilai_ujian_pilgan_siswas.id_ujian')
               ->on('pengambilan_ujians.nisn_siswa', '=', 'nilai_ujian_pilgan_siswas.nisn_siswa');
      })
      ->select('pengambilan_ujians.*', 'ujians.judul_ujian', 'ujians.jenis_ujian', 'ujians.tgl_ujian', 'mata_pelajarans.nama_mapel', 'nilai_ujian_pilgan_siswas.id_nilai_ujian_pilgan')
      ->where('pengambilan_ujians.nisn_siswa', $nisn)
      ->orderBy('pengambilan_ujians.diambil_pada', 'desc')
      ->get();


    // Ambil semua id_nilai_ujian_pilgan milik siswa
    $idNilaiUjianPilgan = \App\NilaiUjianPilihanGandaSiswa::where('nisn_siswa', $nisn)->pluck('id_nilai_ujian_pilgan');
    $userJawabLembar = \App\SiswaJawabUjianPilihanGanda::whereIn('id_nilai_ujian_pilgan', $idNilaiUjianPilgan)->get();

        return view('admin.dashboard.ujian.ujian',$data)
          ->with('siswa', $siswa)
          ->with('userJawabLembars', $userJawabLembar)
          ->with('userAmbilUjians', $userAmbilUjians)
...
public function tambah(Request $request)
  {
        ...
        $pesan = array(
            ...
            'jam_mulai.required'        => 'Jam Mulai Ujian dibutuhkan.',
            'jam_selesai.required'      => 'Jam Selesai Ujian dibutuhkan.',
        ...
        $ujian = new Ujian;
        // Batasi jenis ujian hanya ke tiga opsi
        $allowedJenis = ['Ujian Harian', 'Ujian Tengah Semester', 'Ujian Akhir Semester'];
        $jenis = $request['jenis_ujian'];
        if (!in_array($jenis, $allowedJenis)) {
          return Redirect::back()->withErrors(['jenis_ujian' => 'Jenis ujian tidak valid'])->withInput();
        }
        $ujian->jenis_ujian = $jenis;
        ...
        // Hitung durasi dalam menit dari jam_mulai dan jam_selesai
        $mulai = strtotime($request['jam_mulai']);
        $selesai = strtotime($request['jam_selesai']);
        $ujian->waktu_ujian       = ($selesai - $mulai) / 60;
        ...
        $ujian->nilai_visible     = false; // Default tersembunyi, guru/admin harus aktifkan manual
...
public function simpanedit($id_ujian)
    {
        ...
        // Batasi jenis ujian hanya ke tiga opsi saat edit
        $allowedJenis = ['Ujian Harian', 'Ujian Tengah Semester', 'Ujian Akhir Semester'];
        $jenis = $input['jenis_ujian'];
        if (!in_array($jenis, $allowedJenis)) {
          return Redirect::back()->withErrors(['jenis_ujian' => 'Jenis ujian tidak valid'])->withInput();
        }
        $editUjian->jenis_ujian = $jenis;
        $editUjian->judul_ujian     	= $input['judul_ujian'];
        $editUjian->kelas_ujian     	= $input['kelas_ujian'];
        $editUjian->jam_mulai         = $input['jam_mulai'];
        $editUjian->jam_selesai       = $input['jam_selesai'];
        // Hitung durasi dalam menit dari jam_mulai dan jam_selesai
        $mulai = strtotime($input['jam_mulai']);
        $selesai = strtotime($input['jam_selesai']);
        $editUjian->waktu_ujian     	= ($selesai - $mulai) / 60;
    ...
    public function store_siswa()
    {
        ...
        if (!$siswa) {
            return abort(403, 'Forbidden: Only students can take exams.');
        }
...
public function show($id_ujian)
  {
    try {
        ...
        $siswa = Siswa::where('nisn_siswa', $nilaiUjianPilganSiswa->nisn_siswa)->first();
        if (!$siswa) {
            Session::flash('flash_message', 'Data siswa tidak ditemukan');
            return Redirect::back();
        }
