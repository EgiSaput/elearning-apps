...
public function index()
  {
    ...
    if (Auth::user()->level == 11) {
      ...
      // Add max_poin for each question
      foreach ($dataSoalUjian as $soal) {
        $soal->max_poin = DB::table('jawaban_soal_ujians')
  ...
  {
    $soal = Soal::where('id_soal', '=', $id_soal)->first();
    if (!$soal) {
      app::abort(404);
    }
    Session::flash('flash_message', 'Data Soal Ujian "'.$soal->jenis_soal.'" - "'.$soal->no_soal.'" Berhasil dihapus.');
    $soal->delete();
    if (Auth::user()->level == 11) {
      return redirect('admin/ujian/'.$soal->id_ujian.'/detail');
    }elseif (Auth::user()->level == 12) {
      return redirect('guru/ujian/'.$soal->id_ujian.'/detail');
    }
          ...
          if (!empty($gambar)) {
            ...
            if($validator->fails())
              # Kembali kehalaman yang sama dengan pesan error
                ...
                $jawaban1->is_benar = (int)$request->input('a_is_benar');
                $jawaban1->poin     = (int)$request->input('Point_Jawaban_A');
                ...
                $jawaban2->is_benar = (int)$request->input('b_is_benar');
                $jawaban2->poin     = (int)$request->input('Point_Jawaban_B');
                ...
                $jawaban3->is_benar = (int)$request->input('c_is_benar');
                $jawaban3->poin     = (int)$request->input('Point_Jawaban_C');
                ...
                $jawaban4->is_benar = (int)$request->input('d_is_benar');
                $jawaban4->poin     = (int)$request->input('Point_Jawaban_D');
 ...
 public function edit($id_soal)
    {        
        ...
        $dataUjian = DB::table('ujians')   
          ...
          $guru = Guru::where('id_user', Auth::user()->id_user)->first();
          if (!$guru) {
            return Redirect::back()->with('flash_message', 'Data guru tidak ditemukan.');
          }
          $mapels = $guru->mataPelajaran()->get()->pluck('id_mapel')->toArray();
          if (count($mapels)) {
            $dataUjian = DB::table('ujians')   
                 ->join('mata_pelajarans','ujians.id_mapel','=','mata_pelajarans.id_mapel')
                 ->select('ujians.*', 'mata_pelajarans.nama_mapel')
  ...
  public function simpanedit(Request $request, $id_soal)
    {
      ...
      $jenis_soal = isset($input['jenis_soal']) ? $input['jenis_soal'] : null;
   ...
   /**
    ...
    public function show($userjawablembarId, $soalId = 0)
    {
        try {
          ...
          // Set wkt_mulai jika masih null
          if (!$userJawabLembar->wkt_mulai) {
          ...
          $id_user_siswa = Siswa::where('nisn_siswa', $userJawabLembar->nisn_siswa)->first()->id_user;
            ...
            // Hitung maxTime berdasarkan jam_selesai ujian
            $jamSelesai = $ujian->jam_selesai ?? '23:59:59';
            $maxTimeFromJam = strtotime($ujian->tgl_ujian . ' ' . $jamSelesai);
            
            // Jika jam_selesai <= jam_mulai, berarti jam selesai di hari berikutnya
            $jamMulai = $ujian->jam_mulai ?? '00:00:00';
            $waktuMulaiUjian = strtotime($ujian->tgl_ujian . ' ' . $jamMulai);
            if ($maxTimeFromJam <= $waktuMulaiUjian) {
              $maxTimeFromJam = strtotime('+1 day', $maxTimeFromJam);
            }
            
            // Gunakan waktu selesai yang lebih awal antara: 
            // 1. Waktu mulai siswa + durasi ujian (waktu_ujian menit)
            // 2. Jam selesai ujian yang ditetapkan guru
            $maxTimeFromDurasi = strtotime('+' . $waktu_ujian . ' minutes', $startTime);
            $maxTime = min($maxTimeFromDurasi, $maxTimeFromJam);
            ...
            $soalData = Soal::find($soalId);
            if (!$soalData) {
                Session::flash('flash_message', 'Soal dengan id ' . $soalId . ' tidak ditemukan');
                return Redirect::back();
            }
    ...
    {       
      ...
      //go to next soal
        try {
          ...
          // Gunakan waktu selesai yang lebih awal
          $maxTimeFromDurasi = strtotime('+' . $waktu_ujian . ' minutes', $startTime);
          $maxTime = min($maxTimeFromDurasi, $maxTimeFromJam);
          ...
          if (time() > $maxTime) {
              ...
              if (!$userJawabLembar->wkt_selesai) {
                  $userJawabLembar->wkt_selesai = date('Y-m-d H:i:s');
                  // Hitung nilai final dengan soal yang sudah dijawab
                  $userJawabLembar->calcScore();
                  $userJawabLembar->save();
              }
              Session::flash('flash_message', 'Waktu mengerjakan sudah habis. Ujian telah disubmit otomatis. Soal yang tidak dijawab dianggap salah.');
          ...
          if ($soal->jenis_soal == 'Pilihan Ganda') {
              if ($request->input('jawaban')) {
                  $jawaban = (int)$request->input('jawaban');
              ...
              // If no answer selected, skip saving (allow skipping)
              ...
              if (!$jawabanEssay) {
                  return Redirect::action('Admin\\SoalUjianController@show', array($userJawabLembar->id_nilai_ujian_pilgan, $soalId))->with('messages',
                      array(
                          array('error', 'Jawaban essay harus diisi')
                      ));
              }
              $userJawab = SiswaJawabUjianPilihanGanda::whereRaw('id_nilai_ujian_pilgan = ? and id_soal = ? and id_jawaban_soal_ujian is null', array($userJawabLembar->id_nilai_ujian_pilgan, $soal->id_soal))->first();
              if (!$userJawab){
              ...
              $userJawab->id_jawaban_soal_ujian = null; // null to mark as answered for essay
              $userJawab->jawaban_essay = $jawabanEssay;
              $userJawab->save();
          }
          ...
          DB::commit();
            ...
            // Jika soal terakhir, set waktu selesai dan redirect ke halaman ujian siswa
            $userJawabLembar->wkt_selesai = date('Y-m-d H:i:s');
            $userJawabLembar->save();
            Session::flash('flash_message', 'Jawaban ujian berhasil disubmit.');
            return redirect('/siswa/ujian');
