...
<html>
  ...
  <title>{{ config('app.name', 'E-learning') }}</title>
    ...
    <link rel="icon" type="image/png" href="{{ asset('auth/images/logo_sklh.png?v=').time() }}">
<link rel="shortcut icon" type="image/png" href="{{ asset('auth/images/logo_sklh.png?v=').time() }}">
    ...
    <!-- jika guru maka pake layout-top-nav -->
    @if(Auth::user()->level==12)
    <body class="layout-fixed sidebar-mini skin-red">
    @endif

    <div class="wrapper">
      @include('admin.include.header_siswa_guru')
