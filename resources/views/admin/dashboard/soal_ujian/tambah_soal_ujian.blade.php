...
@section('content')
    ...
    <div class="box box-danger">
      ...
      <div class="pull-right"> 
        @if(Auth::user()->level == 12)
          <a href="{{ url('guru/soal_ujian') }}" class="btn btn-success btn-xs">
    ...
    <div style="display: block;" class="box-body">
      @if(Auth::user()->level == 12)
        <form id="formSoalUjianTambah" class="form-horizontal" role="form" method="POST" enctype="multipart/form-data" action="{{ url('guru/soal_ujian/tambah') }}" >
      @else
                  ...
                  <div class="col-md-12">                    
                     <label for="id_ujian">ID Ujian</label>          
                       <select class="form-control " name="id_ujian">
                        <option value="{{ old('id_ujian') ?: request()->get('id_ujian') }}">-- Pilih ID Ujian --</option>
                        @foreach ($ujian as $idUjian)
                          <option value="{{ $idUjian->id_ujian }}" {{ ((string)old('id_ujian') === (string)$idUjian->id_ujian || (string)request()->get('id_ujian') === (string)$idUjian->id_ujian) ? 'selected' : '' }}>id ujian : {{ $idUjian->id_ujian }} | {{ $idUjian->jenis_ujian }} | {{ $idUjian->nama_mapel }} </option>
                        @endforeach                                                                                 
                       </select>
                    </div>
                  </div>
              ...
              <div class="form-group"> 
                 <div class="col-md-8 col-md-offset-5">
                   <button type="submit" class="btn btn-primary" id="button-reg">
                    ...
                    @if(Auth::user()->level == 12)
                      <a href="{{ url('guru/soal_ujian') }}" title="Cancel">
